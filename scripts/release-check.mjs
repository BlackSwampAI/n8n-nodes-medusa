...
// n8n accepts either an unscoped n8n-nodes-* name or a scoped @scope/n8n-nodes-* name.
if (!/^(@[a-z0-9][a-z0-9._-]*\/)?n8n-nodes-[a-z0-9][a-z0-9._-]*$/.test(packageJson.name ?? '')) {
	fail('package.json name must be a lowercase n8n-nodes-* or @scope/n8n-nodes-* package name');
}
